...
export async function searchYoutubeTracks(query: string, limit = 10): Promise<YoutubeSearchResult[]> {
  try {
    ...
    return limitedSongs.map((v: any) => ({
      ...
      thumbnail: v.thumbnails?.[0]?.url || v.thumbnail?.contents?.[0]?.url || '',
 ...
 */
export async function getArtistDetails(artistIdOrName: string): Promise<ArtistDetails | null> {
  try {
    const yt = await getClient();
    ...
    if (!channelId.startsWith('UC') && !channelId.startsWith('FE')) {
      ...
      const artistSearch = await yt.music.search(artistIdOrName, { type: 'artist' });
    ...
    if (Array.isArray(headerThumb) && headerThumb.length > 0) {
      thumbnail = headerThumb[headerThumb.length - 1]?.url || headerThumb[0]?.url || '';
    }
    ...
    if (rawSongs.length === 0 && anyArtist.sections) {
      const songsSec = anyArtist.sections.find((s: any) =>
        (s.header?.title?.toString() || s.title?.toString())?.toLowerCase()?.includes('song')
      );
    ...
    const topSongs = rawSongs.slice(0, 15).map((s: any) => ({
      ...
      albumArt: s.thumbnails?.contents?.[0]?.url || s.thumbnail?.contents?.[0]?.url || s.thumbnails?.[0]?.url || thumbnail || '',
    ...
    const singlesSec = anyArtist.sections?.find((s: any) =>
      (s.header?.title?.toString() || s.title?.toString())?.toLowerCase()?.includes('single')
    );
    const singles = (singlesSec?.contents || []).slice(0, 12).map((item: any) => {
      const itemThumb = item.thumbnail?.[0]?.url || item.thumbnail?.contents?.[0]?.url || item.thumbnails?.[0]?.url || '';
      return {
        id: item.id,
    ...
    const albumsSec = anyArtist.sections?.find((s: any) =>
      (s.header?.title?.toString() || s.title?.toString())?.toLowerCase()?.includes('album')
    );
    ...
    logger.error(`Failed to fetch artist details for ${artistIdOrName}:`, err.message);
    return null;
  }
}
...
export async function getAlbumDetails(albumId: string): Promise<AlbumDetails | null> {
  try {
    ...
    if (isPlaylist) {
      ...
      const thumb = anyPHeader.thumbnail?.contents?.[0]?.url || anyPHeader.thumbnails?.[0]?.url || '';
      ...
      const tracks = items.map((t: any, idx: number) => ({
        ...
        albumArt: t.thumbnail?.contents?.[0]?.url || t.thumbnails?.[0]?.url || thumb,
    ...
    const title = anyHeader.title?.toString() || anyAlbum.title?.toString() || 'Unknown Album';
    ...
    const authorName = anyHeader.author?.name || 
      straplineRun?.text || 
      anyHeader.strapline_text_one?.toString() || 
      anyAlbum.artists?.[0]?.name || 
      'Unknown Artist';

    const authorId = anyHeader.author?.channel_id || 
    ...
    const thumbnail = Array.isArray(thumbContents) && thumbContents.length > 0
 ...
 */
export async function searchCategorized(query: string, type: 'all' | 'song' | 'album' | 'artist' | 'playlist' = 'all') {
  try {
    const yt = await getClient();
    ...
    if (type === 'song') {
      ...
      const songs = (res.songs?.contents || []).map((v: any) => ({
        ...
        albumArt: v.thumbnails?.[0]?.url || v.thumbnail?.contents?.[0]?.url || '',
        duration: v.duration?.seconds || 0,
        explicit: false,
        isStreamable: true,
    ...
    if (type === 'album') {
      ...
      const albums = (res.albums?.contents || []).map((a: any) => ({
        ...
        thumbnail: a.thumbnail?.[0]?.url || a.thumbnail?.contents?.[0]?.url || a.thumbnails?.[0]?.url || '',
        type: 'album',
      }));
      return { albums };
    }
    ...
    if (type === 'artist') {
      ...
      const artists = (res.artists?.contents || []).map((ar: any) => ({
        ...
        thumbnail: ar.thumbnail?.contents?.[0]?.url || ar.thumbnails?.[0]?.url || '',
      }));
      return { artists };
    }
    ...
    if (type === 'playlist') {
      ...
      const playlists = (res.playlists?.contents || []).map((p: any) => ({
        ...
        thumbnail: p.thumbnail?.contents?.[0]?.url || p.thumbnails?.[0]?.url || '',
      }));
      return { playlists };
    }
    ...
    const songs = songRes.status === 'fulfilled' ? (songRes.value.songs?.contents || []).slice(0, 10).map((v: any) => ({
      provider: 'youtube',
      providerTrackId: v.id,
      title: v.title?.toString() || 'Unknown Title',
      artist: v.artists?.[0]?.name || 'Unknown Artist',
    ...
    const albums = albumRes.status === 'fulfilled' ? (albumRes.value.albums?.contents || []).slice(0, 6).map((a: any) => ({
      id: a.id,
      title: a.title?.toString() || 'Unknown Album',
      artist: a.artists?.[0]?.name || a.author?.name || 'Unknown Artist',
      artistId: a.artists?.[0]?.channel_id || '',
    ...
    const artists = artistRes.status === 'fulfilled' ? (artistRes.value.artists?.contents || []).slice(0, 6).map((ar: any) => ({
      id: ar.id,
      name: ar.name || ar.title?.toString() || 'Unknown Artist',
      subscribers: ar.subscribers?.toString() || '',
      thumbnail: ar.thumbnail?.contents?.[0]?.url || ar.thumbnails?.[0]?.url || '',
    ...
    const playlists = playlistRes.status === 'fulfilled' ? (playlistRes.value.playlists?.contents || []).slice(0, 6).map((p: any) => ({
      id: p.id,
      title: p.title?.toString() || 'Curated Playlist',
      author: p.author?.name || p.authors?.[0]?.name || 'YouTube Music',
      itemCount: p.item_count || '',
